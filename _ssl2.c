/* SSL socket module
   ...
   This module is imported by ssl2.py. It should *not* be used
   directly.
...
#define ERRSTR(x) ERRSTR1("_ssl2.c", STRINGIFY2(__LINE__), x)
...
PySSL_SetError(PySSLObject *obj, int ret, char *filename, int lineno)
{
    ...
    PyOS_snprintf(buf, sizeof(buf), "_ssl2.c:%d: %s", lineno, errstr);
    ERR_clear_error();
...
init_ssl2(void)
{
    ...
    PySSLErrorObject = PyErr_NewException("ssl2.SSLError",
